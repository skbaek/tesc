...
in Agda \cite{bove2009brief}. The aim of the paper is twofold. Its primary purpose is 
to demonstrate the reliability of TESC proofs by showing precisely what is established 
by their successful verification using VTV. Its secondary aim is to serve as a guide 
for understanding VTV's codebase and its design choices, which can be especially useful 
for readers who want to implement their own TESC verifiers.
...
and constants \cite{kumar2014hol}. More recent projects go 
beyond the operational semantics of programming languages and verifies theorem provers 
...
\cite{carneiro2019metamath} theorem prover verified down to x84-64 instruction set architecture. 
...
Ivy system \cite{mccune2000ivy}. The largest difference between Ivy and 
...
preprocessing steps like Skolemization and CNF normalization, so 
it allows ATPs to work directly on input problems with their optimized 
...
caveat. A first instinct definition of terms might be something like the following:
\ExecuteMetaData[wrongterm.tex]{wrongterm}
But this definition works poorly in practice, since any function defined by
structural recursion on terms runs into non-termination issues. For instance, 
substitution on terms must make recursive calls with arguments of functors (the members of the 
list argument to \AgdaInductiveConstructor{fun}), but Agda can't automatically ascertain 
the termination of these calls because those terms are behind a \AgdaFunction{List} 
and hence not immediate subterms. Although it is possible to manually prove termination, 
it is much simpler to sidestep 
this issue with a pseudo-mutual recursion. The basic idea is to use a special type 
\AgdaFunction{Term*} \AgdaSymbol{:} \AgdaFunction{Bool} \AgdaSymbol{→} \AgdaFunction{Set}
that can double as both terms and lists of terms depending on its boolean argument. 
I.e., we use \AgdaFunction{Term*} \AgdaInductiveConstructor{false} as the type of terms, 
and \AgdaFunction{Term*} \AgdaInductiveConstructor{true} as the type of lists of terms.
