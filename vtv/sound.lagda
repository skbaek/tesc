...
term*-lfi< : ∀ {b} → Nat → Term* b → Set
term*-lfi< {false} _ (var _) = ⊤ 
term*-lfi< {false} k (fun f ts) = fi< k f × term*-lfi< k ts
term*-lfi< {true} _ nil = ⊤ 
term*-lfi< {true} k (cons t ts) = term*-lfi< k t × term*-lfi< k ts 
...
term*-lfi<s : ∀ {b} k (f : Term* b) → term*-lfi< k f → term*-lfi< (suc k) f
term*-lfi<s {false} _ (var _) _ = tt 
term*-lfi<s {false} k (fun f ts) h0 = 
  fi<s k f (fst h0) , term*-lfi<s k ts (snd h0) 
term*-lfi<s {true} _ nil _ = tt
term*-lfi<s {true} k (cons t ts) h0 = 
  term*-lfi<s _ t (fst h0) , term*-lfi<s k ts (snd h0)

formula-lfi<-suc : ∀ k f → formula-lfi< k f → formula-lfi< (suc k) f
formula-lfi<-suc k (cst _) h0 = tt 
formula-lfi<-suc k (rel r ts) h0 = 
   ( fi<s _ _ (fst h0) , 
     term*-lfi<s _ _ (snd h0) ) 
...
subst-term*-lfi< : ∀ {b} s (t : Term* b) k m → term*-lfi< k s → 
  term*-lfi< k t → term*-lfi< k (subst-term* m s t)
subst-term*-lfi< {true} _ nil _ _ _ _ = tt
subst-term*-lfi< {true} s (cons t ts) k m h0 h1 = 
  (subst-term*-lfi< s _ _ _ h0 (fst h1)) ,
  (subst-term*-lfi< s _ _ _ h0 (snd h1)) 
subst-term*-lfi< {false} s (var n) k m h0 h1 = 
  tri-intro-lem m n (term*-lfi< k) (λ _ → tt) (λ _ → h0) (λ _ → tt)
subst-term*-lfi< {false} s (fun f ts) k m h0 h1 = 
  (fst h1) , (subst-term*-lfi< _ ts k m h0 (snd h1))

incr-var-lfi< : ∀ {b} k (t : Term* b) → term*-lfi< k t → term*-lfi< k (incr-var t)
incr-var-lfi< _ nil _ = tt
incr-var-lfi< k (cons t ts) h = 
  (incr-var-lfi< _ t (fst h)) , (incr-var-lfi< _ ts (snd h))
incr-var-lfi< k (var _) _ = tt
incr-var-lfi< k (fun f ts) h = 
  fst h , (incr-var-lfi< _ ts (snd h))
...
subst-form-lfi< t (rel r ts) k m h0 h1 = 
  fst h1 , subst-term*-lfi< t ts k m h0 (snd h1)
...
subst-form-lfi< t (qtf _ f) k m h0 h1 = 
  subst-form-lfi< (incr-var t) f k (suc m) (incr-var-lfi< _ _ h0) h1
...
term*-lfi<! : ∀ {b} k (t : Term* b) → T (term*-lfi<? k t) → term*-lfi< k t 
term*-lfi<! {true} _ nil _ = tt
term*-lfi<! {true} k (cons t ts) h0 = 
 term*-lfi<! _ _ (∧-lft  _ _ h0) , term*-lfi<! _ _ (∧-rgt _ _ h0)
term*-lfi<! {false} k (var m) h0 = tt
term*-lfi<! {false} k (fun f ts) h0 =
 fi<! _ _ (∧-lft  _ _ h0) , 
 term*-lfi<! _ _ (∧-rgt _ _ h0)
...
term*-val-subst : ∀ (F : FA) (V : VA) (k : Nat) (b : Bool) (s : Term) (t : Term* b) → 
  (term*-val F (V / k ↦ term*-val F V s) t) ≡ (term*-val F V (subst-term* k s t))
term*-val-subst F V k true s nil = refl
term*-val-subst F V k true s (cons t ts) = 
  cong-2 _∷_ (term*-val-subst F V k false s t) 
    (term*-val-subst F V k true s ts)
term*-val-subst F V k false s (var m) = 
  tri-intro-lem k m 
...
term*-val-subst F V k false s (fun f ts) = 
  cong (F f) (term*-val-subst F V k true _ ts)
...
term*-val-incr : ∀ b F V d (t : Term* b) → term*-val F (V / 0 ↦ d) (incr-var t) ≡ term*-val F V t 
term*-val-incr false F V d (var k) = refl
term*-val-incr false F V d (fun f ts) = 
  cong (F f) (term*-val-incr true F V d ts)
term*-val-incr true  F V d nil = refl
term*-val-incr true  F V d (cons t ts) = 
  cong-2 _∷_ 
    (term*-val-incr false F V d t) 
    (term*-val-incr true F V d ts)

holds-subst : ∀ R F V k t f → 
  ((R , F , (V / k ↦ (term*-val F V t)) ⊨ f) ↔ (R , F , V ⊨ subst-form k t f))
holds-subst R F V k t (rel r ts) = 
  eq-to-iff (λ x → T (R r x)) _ _ (term*-val-subst F V k true _ ts)
...
term*-val-update-par : ∀ F k d V → 
  term*-val (F / indexed k ↦f const-fun d) V (par k) ≡ d
term*-val-update-par F k d V = 
...
lfi-lt-to-term*-val-eq : ∀ {b} F V k fn (t : Term* b) → (term*-lfi< k t) → 
  (term*-val (F / indexed k ↦f fn) V t) ≡ (term*-val F V t) 
lfi-lt-to-term*-val-eq {true} F V k fn nil h0 = refl
lfi-lt-to-term*-val-eq {true} F V k fn (cons t ts) h0 = 
  cong-2 _∷_
    (lfi-lt-to-term*-val-eq F V k fn t (fst h0))  
    (lfi-lt-to-term*-val-eq F V k fn ts (snd h0))
...
lfi-lt-to-holds-ru-iff R F V k rl (rel r ts) h0 = 
  eq-to-iff (λ (x : Rels) → T (x (term*-val F V ts))) ((R / (indexed k) ↦r rl) r) (R r) 
...
lfi-lt-to-holds-update-iff R F V k fn (rel r ts) h0 = 
  eq-to-iff (λ x → T (R r x)) _ _ (lfi-lt-to-term*-val-eq F V k fn ts (snd h0)) 
...
term*=! : ∀ {b} (t s : Term* b) → T (term*=? t s) → t ≡ s
term*=! nil nil _ = refl
term*=! (cons t0 ts0) (cons t1 ts1) h0 = 
  let (h1 , h2) = ∧⇒× (term*=? t0 _) _ h0 in 
  cong-2 cons (term*=! _ _ h1) (term*=! _ _ h2)
term*=! (var k) (var m) h0 = cong var (≡ᵇ⇒≡ _ _ h0)
term*=! (fun f0 ts0) (fun f1 ts1) h0 = 
  let (h1 , h2) = ∧⇒× (functor=? f0 _) _ h0 in 
  cong-2 fun (functor=! _ _ h1) (term*=! _ _ h2)

terms=! : ∀ (t s : Terms) → T (term*=? t s) → t ≡ s
terms=! = term*=! {true}
...
formula=! (rel r0 ts0) (rel r1 ts1) h0 = 
  let (h1 , h2) = ∧⇒× _ _ h0 in
  cong-2 rel (functor=! _ _ h1) (term*=! _ _ h2)
...
from-mono-args-equal-0 : ∀ F V k → mono-args-equal' k V → 
  (term*-val F V (mono-args-lft k) ≡ term*-val F V (mono-args-rgt k)) 
...
term*-vars< : ∀ {b} → Nat → Term* b → Set
term*-vars< {true} _ nil = ⊤
term*-vars< {true} k (cons t ts) = 
  term*-vars< k t × term*-vars< k ts 
term*-vars< {false} k (var m) = m < k
...
eq-va-lt-to-eq : ∀ {b} F V0 V1 k (t : Term* b) → eq-va-lt k V0 V1 → term*-vars< k t → 
  (term*-val F V0 t) ≡ (term*-val F V1 t) 
...
eq-va-lt-to-iff R F V0 V1 k (rel r ts) h0 h1 = 
  eq-to-iff (λ x → T (R r x)) (term*-val F V0 ts) _ (eq-va-lt-to-eq F V0 V1 k ts h0 h1)
...
reverse-trunc-eq-term*-val-vars-desc-core : ∀ F V m → 
  term*-val F (↓ V) (vars-desc m) ∷ʳ V 0 ≡ term*-val F V (cons (var m) (vars-desc m))
reverse-trunc-eq-term*-val-vars-desc-core F V 0 = refl 
reverse-trunc-eq-term*-val-vars-desc-core F V (suc m) = cong-2 _∷_ refl (reverse-trunc-eq-term*-val-vars-desc-core F _ m) 

term*-val-rev-terms : ∀ F V ts0 ts1 → 
  term*-val F V (rev-terms ts0 ts1) ≡  reverseAcc (term*-val F V ts1) (term*-val F V ts0) 
term*-val-rev-terms F V nil ts1 = refl 
term*-val-rev-terms F V (cons t ts0) ts1 = term*-val-rev-terms F V ts0 (cons t ts1) 

reverse-trunc-eq-term*-val-vars-desc : ∀ F V m → reverse (trunc m V) ≡ term*-val F V (vars-desc m)
reverse-trunc-eq-term*-val-vars-desc F V 0 = refl
reverse-trunc-eq-term*-val-vars-desc F V (suc m) = eq-trans _ (reverse-cons (V 0) (trunc m (↓ V))) 
  (eq-trans ((term*-val F (↓ V) (vars-desc m)) ∷ʳ V 0) 
  (cong (λ x → x ∷ʳ V 0) (reverse-trunc-eq-term*-val-vars-desc F (↓ V) m)) (reverse-trunc-eq-term*-val-vars-desc-core F _ m))
...
data only-vars : ∀ {b} → Term* b → Set where 
...
only-vars-to-eq : ∀ {b} F0 F1 V (t : Term* b) → only-vars t → term*-val F0 V t ≡ term*-val F1 V t
...
val-vars-asc-eq : ∀ F0 F1 V k → term*-val F0 V (vars-asc k) ≡ term*-val F1 V (vars-asc k) 
val-vars-asc-eq F0 F1 V k = only-vars-to-eq _ _ _ _  (only-vars-vars-asc k)

val-vars-desc-eq : ∀ F0 F1 V k → term*-val F0 V (vars-desc k) ≡ term*-val F1 V (vars-desc k) 
...
skm-fun-desc-reverse-trunc R F V f k m = 
  eq-trans _ 
    (cong (skm-fun-desc R F f) {reverse (trunc m V)} (reverse-trunc-eq-term*-val-vars-desc F V m))
    ( cong-fun-arg {_} {_} {skm-fun-desc R F f} {_} 
        {term*-val F V (vars-desc m)} {term*-val _ V (vars-desc m)} 
...
trunc-eq-term*-val-vars-asc : ∀ F V m → (trunc m V) ≡ term*-val F V (vars-asc m)
trunc-eq-term*-val-vars-asc F V m = 
  reverse-inj _ _ 
    ( eq-trans _ (reverse-trunc-eq-term*-val-vars-desc F V m) 
        ( eq-trans _
            (eq-symm (reverse-reverse (term*-val F V (vars-desc m)))) 
            ( cong reverse {_} {term*-val F V (vars-asc m)} 
                (eq-symm (term*-val-rev-terms F V (vars-desc m) nil)) ) ) )

rev-terms-lfi< : ∀ k ts0 ts1 → term*-lfi< k ts0 → term*-lfi< k ts1 → term*-lfi< k (rev-terms ts0 ts1)
rev-terms-lfi< k nil ts1 _ h0 = h0 
rev-terms-lfi< k (cons t ts0) ts1 (h0 , h1) h2 = rev-terms-lfi< k ts0 (cons t ts1) h1 (h0 , h2) 

vars-desc-lfi< : ∀ k m → term*-lfi< k (vars-desc m) 
vars-desc-lfi< k 0 = tt
vars-desc-lfi< k (suc m) = tt , (vars-desc-lfi< _ _)

vars-asc-lfi< : ∀ k m → term*-lfi< k (vars-asc m) 
vars-asc-lfi< k m = rev-terms-lfi< k (vars-desc m) nil (vars-desc-lfi< _ _) tt
...
prsv-t-pred-def R F k m _ (pred-def-iff-asc k m f h0 h1) = 
  def-rl-asc R F f , λ V → iff-trans (T (def-rl-asc R F f ((trunc m V)))) 
    ( eq-to-iff-2 (λ x y → T (x y)) ((R / (indexed k) ↦r _) (indexed k)) (def-rl-asc R F f) _ (trunc m V) 
        (ra-update-eq R k _) (eq-symm (trunc-eq-term*-val-vars-asc F V m)) ) 
  ...
  def-rl-desc R F f , λ V → iff-trans (T (def-rl-desc R F f (reverse (trunc m V)))) 
    (eq-to-iff-2 (λ x y → T (x y)) ((R / indexed k ↦r _) (indexed k))
      (def-rl-desc R F f) _ (reverse (trunc m V)) (ra-update-eq R k _) 
        (eq-symm (reverse-trunc-eq-term*-val-vars-desc F V m))) (iff-trans _ tr-rt-iff 
...
skm-fun-asc-trunc R F V f k m = 
  eq-trans _ 
    (cong (skm-fun-asc R F f) {trunc m V} (trunc-eq-term*-val-vars-asc F V m))
    ( cong-fun-arg {_} {_} {skm-fun-asc R F f} {_} 
        {term*-val F V (vars-asc m)} {term*-val _ V (vars-asc m)} 
        (fa-update-eq F k _) 
...
mono-args-lft-lfi< : ∀ k m → term*-lfi< k (mono-args-lft m)
mono-args-lft-lfi< k 0 = tt
mono-args-lft-lfi< k (suc m) = tt , (mono-args-lft-lfi< k m)

mono-args-rgt-lfi< : ∀ k m → term*-lfi< k (mono-args-rgt m)
...
mono-fun! k m (qtf false (qtf false (bct imp (rel (plain (c ∷ [])) (cons (var 1) (cons (var 0) nil))) f))) h0 = 
  let h1 = mono-fun! k (suc m) f (∧-rgt (c =c '=') _ h0) in 
  let h2 = char=! _ _ (∧-lft  (c =c '=') _ h0) in 
  eq-elim' 
    ( λ x →
        mono-fun k m
          (∀* (∀* (rel (plain (x ∷ [])) (cons (var 1) (cons (var zero) nil)) →* f))) )
    (eq-symm h2) (mono-fun-fa k m f h1) 
mono-fun! k m (rel (plain (c ∷ [])) (cons (fun f0 ts0) (cons (fun f1 ts1) nil))) ht0 = 
  let (h1 , h2 , h3 , h4 , h5) = ∧∧∧∧⇒×××× (c =c '=') _ _ _ _ ht0 in 
  eq-elim'-4 (λ x y z w → mono-fun k m (rel (plain (x ∷ [])) (cons (fun f0 y) (cons (fun z w) nil))))
    (eq-symm (char=! _ _ h1)) 
    (eq-symm (term*=! _ _ h4)) 
    (functor=! f0 f1 h3) 
    (eq-symm (term*=! _ _ h5)) 
...
mono-rel! k m (qtf false (qtf false (bct imp (rel (plain (c ∷ [])) (cons (var 1) (cons (var 0) nil))) f))) h0 = 
...
term*-vars<! : ∀ {b} k (t : Term* b) → T (term*-vars<? k t) → term*-vars< k t
term*-vars<! k nil h0 = tt
term*-vars<! k (cons t ts) h0 = 
  let (h1 , h2) = ∧⇒× _ _ h0 in 
  term*-vars<! k t h1 , term*-vars<! k ts h2
term*-vars<! k (var m) h0 = <ᵇ⇒< _ _ h0 
term*-vars<! k (fun _ ts) h0 = term*-vars<! k ts h0
...
formula-vars<! k (rel _ ts) h0 = term*-vars<! k ts h0
