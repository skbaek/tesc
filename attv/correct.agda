...
mem-nth B k = or-elim (mem-lookup B (suc k) ⊤*) or-lft or-rgt
...
good-bch : Bch → Set
good-bch B = ∀ f → mem f B → good-form (len B) f

best-bch : Bch → Set
best-bch B = ∀ f k → mem f B → good-form k f

good-ftr-suc : ∀ k f → good-ftr k f → good-ftr (suc k) f
good-ftr-suc k (nf m) h = <-trans h (n<sn _) -- lt-to-lt-suc m k h
...
good-bch-add : ∀ B f → (0 < size B) → good-bch B → 
  good-form (suc (len B)) f → good-bch (add B f)
good-bch-add B f hL hB hf = 
  let heq : len (add B f) ≡ suc (len B)
      heq = eq-trans _ (cong pred (size-add  B f)) (pred-suc-eq-suc-pred (size B) hL) in 
  all-add (good-form (len (add B f))) B f 
    (λ g hg → elim-eq-symm (λ x → good-form x g) (good-form-suc (len B) g (hB g hg)) heq) 
    (elim-eq-symm (λ x → good-form x f) hf heq)
    
  --   (λ g hg → good-form-suc (len B) g (hB g hg)) 
  --   hf 
  --   -- {! all-add !} {!   !}

-- good-bch-add (fork _ t s) f hB hf = {!   !}
  -- or-elim' (\ h2 → eq-elim _ (eq-symm h2) h0) (\ h2 → good-form-suc _ g (h1 _ h2)) 

-- good-prob-nil : good-bch (leaf ⊤*)
-- good-bch-nil k f = ? -- (n , ()) 

good-form-nth : ∀ B k → good-bch B → good-form (len B) (nth k B)
good-form-nth B k h = or-elim (mem-nth B k) (h _) 
  λ h1 → eq-elim-symm (good-form (len B)) h1 tt
...
prsv-good-b k (bct imp f g) = id -- (hf , hg) , (hg , hf) 
...
prsv-good-b k (not (bct iff f g)) (hf , hg) = (hf , hg) , (hg , hf) --lid
...
good-bch-a : ∀ B i b → 0 < size B → good-bch B → good-bch (add B (apply-a b (nth i B)))
good-bch-a B i b h0 h1 = 
  good-bch-add B _ h0 h1
    ( good-form-suc _ (apply-a b (nth i B)) 
      (prsv-good-a _ b _ (good-form-nth B i h1)) )

good-bch-b : ∀ B k → 0 < size B → good-bch B → 
  (good-bch (add B (fst (apply-b (nth k B)))) ∧ good-bch (add B (snd (apply-b (nth k B)))))
good-bch-b B k h0 h1 = 
  let (h2 , h3) = prsv-good-b (len B) (nth k B) (good-form-nth B k h1) in
  (good-bch-add B _ h0 h1 (good-form-suc (len B) (fst (apply-b (nth k B))) h2)) , 
  (good-bch-add B _ h0 h1 (good-form-suc (len B) (snd (apply-b (nth k B))) h3))

good-bch-c : ∀ B k t → 0 < size B → good-bch B → good-term (suc (len B)) t → 
  (good-bch (add B (apply-c t (nth k B))))
good-bch-c B k t h0 h1 h2 = good-bch-add B _ h0 h1 
  (prsv-good-c _ t (nth k B) h2 (good-form-suc _ (nth k B) (good-form-nth B k h1))) 

good-bch-d : ∀ B k → 0 < size B → good-bch B → good-bch (add B (apply-d (len B) (nth k B)))
good-bch-d B k h0 h1 = 
  good-bch-add B _ h0 h1 (prsv-good-d (len B) (nth k B) (good-form-nth B k h1))

good-bch-n : ∀ B k → 0 < size B → good-bch B → good-bch (add B (apply-n (nth k B)))
good-bch-n B k h0 h1 = 
  good-bch-add B _ h0 h1 
    (prsv-good-n _ (nth k B) (good-form-suc _ (nth k B) (good-form-nth _ _ h1)))  

chks-good-ftr : ∀ k f → tr (chk-good-ftr k f) → good-ftr k f
chks-good-ftr k (nf m) h = <ᵇ⇒< _ _ h 
chks-good-ftr _ (sf _) _ = tt

chks-good-termoid : ∀ {b} k (t : Termoid b) → tr (chk-good-termoid k t) → good-termoid k t 
...
chks-good-form : ∀ k f → tr (chk-good-form k f) → good-form k f  
...
term-val-update-par F k d V = 
  let h0 = tr-to-ite-eq {List D → D} {k =n k} {λ _ → d} {F (nf k)} (≡⇒≡ᵇ k _ refl) in --(eq-to-nat-eq k k refl) in 
  ...
  ex-elim h1 λ d h2 → 
    ...
    d , 
      ...
      ( elim-eq-symm (λ x → R , F' , V / 0 ↦ x ⊨ f) 
          ...
          (term-val-update-par F k d V) ) -- (elim-eq-symm (λ x → R , F' , V / 0 ↦ x ⊨ f) (term-val-update-par F k d V) ?)
...
sats-to-sats-fa : ∀ R F V B f → good-bch B → 
  sats R F V B → sats R (F / (nf (len B)) ↦f f) V B
sats-to-sats-fa R F V B f h0 h1 g h2 = 
  snd (good-to-holds-update-iff R F V (len B) f g (h0 g h2)) (h1 g h2)

sats-to-sats-ra : ∀ R F V B r → good-bch B → 
  sats R F V B → sats (R / (nf (len B)) ↦r r) F V B
sats-to-sats-ra R F V B r h0 h1 f h2 = 
  snd (good-to-holds-ru-iff R F V (len B) r f (h0 f h2)) (h1 f h2)
  
prsv-d : ∀ B k → good-bch B → sat B → sat (add B (apply-d (len B) (nth k B))) 
prsv-d B k hB h0 = 
  ex-elim-3 h0 
    λ R F V (hR , hs) → 
      ex-elim
        ( prsv-d-aux R F V (len B) (nth k B) 
          (good-form-nth B k hB) 
          (sats-nth R F V B k hs) ) 
        λ d hd → 
          R , F / (nf (len B)) ↦f (const-fun d) , V , hR , 
...
from-mono-args-equal-suc V k h0 m h1 = h0 m (<-trans h1 (n<sn _)) --(lt-to-lt-suc m k h1) 
...
from-mono-args-equal-0 F V (suc k) h0 = 
  cong-2 _∷_ (h0 k (n<sn _)) -- (lt-suc-self k)) 
...
standard-to-sat : ∀ {B f} → standard-to-holds f → sat B → sat (add B f) -- → unsat P B
...
ra-update-eq R k r = (tr-to-ite-eq {_} {ftr-eq (nf k) (nf k)} (≡⇒≡ᵇ k _ refl)) --(≡ᵇ⇒≡ _ _ refl)) 
...
prsv-t : ∀ B f → good-bch B → jst (len B) f → sat B → sat (add B f)
...
prsv-t B f hB (jst-choice k f h0) = 
  ex-elim-3' 
    λ R F V (h1 , h2) → 
      ex-elim (prsv-t-choice R F (len B) 0 f h0) 
        λ fn h3 → 
          R , F / nf (len B) ↦f fn , V , h1 , 
            sats-add R _ V B f (sats-to-sats-fa R F V B fn hB h2) (h3 V)
prsv-t B f hB (jst-pred-def k f h0) = 
  ex-elim-3' λ R F V (h1 , h2) → 
    ex-elim (prsv-t-pred-def R F (len B) 0 f h0)
       λ r h3 → (R / nf (len B) ↦r r) , F , V ,  standard-ru R k r h1 , 
...
good-bch-t : ∀ B g → jst (len B) g → 0 < size B → good-bch B → good-bch (add B g)
good-bch-t B _ (jst-top _) h0 h1           = good-bch-add B _ h0 h1 tt 
good-bch-t B _ (jst-not-bot _) h0 h1       = good-bch-add B _ h0 h1 tt
good-bch-t B _ (jst-refl _) h0 h1          = good-bch-add B _ h0 h1 (tt , tt , tt , tt )
good-bch-t B _ (jst-symm _) h0 h1          = good-bch-add B _ h0 h1 ((tt , tt , tt , tt) , (tt , tt , tt , tt))
good-bch-t B _ (jst-trans _) h0 h1         = good-bch-add B _ h0 h1 ((tt , tt , tt , tt), (tt , tt , tt , tt) , (tt , tt , tt , tt))
good-bch-t B _ (jst-fun k f hm) h0 h1      = good-bch-add B _ h0 h1 (good-mono-fun k 0 f hm)
good-bch-t B _ (jst-rel k f hm) h0 h1      = good-bch-add B _ h0 h1 (good-mono-rel k 0 f hm)
good-bch-t B _ (jst-choice k f hc) h0 h1   = good-bch-add B _ h0 h1 (good-choice (len B) 0 _ hc) 
good-bch-t B _ (jst-pred-def k f hp) h0 h1 = good-bch-add B _ h0 h1 (good-pred-def (len B) 0 _ hp) 
...
from-chks-vars-lt-termoid k (var m) h0 = <ᵇ⇒< _ _ h0 --nat-lt-to-lt _ _ h0
...
chks-jst : ∀ k f → tr (chk-jst k f) → jst k f
chks-jst k f = elim-bor (form-eq f (cst true)) _ 
...
correct-core : ∀ B p → (0 < size B) → good-bch B → tr (verify B p) → unsat B
correct-core B (rule-a b k p) hL hB hp hs = 
   correct-core (add B _) p (pos-size-add B _) 
     (good-bch-a B k b hL hB) hp (prsv-a B b k hs) 
correct-core B (rule-b k p q) hL hB hpq hs = 
  let (hp , hq) = tr-band-to-and (verify _ p) _ hpq in 
  let (h0 , h1) = good-bch-b B k hL hB in
  or-elim (prsv-b B k hs) 
    (correct-core _ p (pos-size-add B _) h0 hp) 
    (correct-core _ q (pos-size-add B _) h1 hq)
correct-core B (rule-c t k p) hL hB hp hs = 
  let (hp0 , hp1) = tr-band-to-and (chk-good-term _ t) _ hp in 
  correct-core (add B _) p (pos-size-add B _) 
    (good-bch-c B k t hL hB (chks-good-termoid _ t hp0)) 
    hp1 (prsv-c B t k hs)
correct-core B (rule-d k p) hL hB hp hs = 
  correct-core (add B _) p (pos-size-add B _) 
    (good-bch-d B k hL hB) hp (prsv-d B k hB hs)
correct-core B (rule-n k p) hL hB hp hs = 
  correct-core (add B _) p (pos-size-add B _) (good-bch-n B k hL hB) hp (prsv-n B k hs)
correct-core B (rule-s f p q) hL hB h0 hs = 
  let (h1 , hpq) = tr-band-to-and (chk-good-form _ f) _ h0 in 
  let (hp , hq) = tr-band-to-and (verify _ p) _ hpq in 
  let hg = chks-good-form _ f h1 in
  or-elim (prsv-s B f hs) 
    (correct-core _ p (pos-size-add B _) (good-bch-add B (not f) hL hB hg) hp) 
    ((correct-core _ q (pos-size-add B _) (good-bch-add B f hL hB hg) hq))
correct-core B (rule-t f p) hL hB h0 hs = 
  let (h1 , hp) = tr-band-to-and (chk-jst _ f) _ h0 in 
  let hj = chks-jst (len B) f h1 in 
  correct-core (add B f) p (pos-size-add B _) 
    (good-bch-t B f hj hL hB) hp (prsv-t B f hB hj hs)
correct-core B (rule-x i j) hL hB hp hs = 
  ex-elim-3 hs 
    λ R F V (hR , hs) → 
...
returns : ∀ {A : Set} → Read A → A → Set
returns r a = ∃ λ cs0 → ∃ λ cs1 → (r cs0) ≡ cont a cs1

correct : ∀ B p → returns read-bch B → tr (verify B p) → unsat B  
correct B p h0 h1 = {!   !}  
      ...
      ex-elim (from-ends-verify-x B h0) 
        λ g (h1 , h2) → 
          elim-lem (R , F , V ⊨ g) 
            ...
            λ h3 → g , or-rgt h1 , h3 )
correct-init : ∀ B p → good-bch B → tr (verify B p) → unsat B
correct-init B (rule-a b i p) hB hp R F V hR =
  {!  correct-init (add B _) p !} -- ex-elim (correct-core P [] k hP pall-nil hp R F V hR) 
...
from-passes-num-verify-s B g (k , cs0 , csf , h0) = 
  use-bind-eq-just (read-form k) _ cs0 csf g h0 
    λ f cs1 h1 h2 → 
      ...
      let h5 = chks-good-form _ f h3 in 
...
from-passes-num-verify-t B g (k , cs0 , csf , h0) = 
  use-bind-eq-just (read-form k) _ cs0 csf g h0 (λ f cs1 h1 h2 → 
    ...
    let h6 = chks-jst (length B) f h3 in 
...
from-passes-verify-p P B g (cs0 , csf , h0) =
  use-bind-eq-just read-chars _ cs0 csf g h0 λ nm cs1 h1 h2 → 
    ...
    eq-elim (λ x → in-prob x P ∧ good-form (suc (length B)) x) h8 (h5 , chks-good-form _ f h6)
                    ...
                    ( elim-eq good-prob 
                        ( good-prob-cons i f Q 
                            ( λ m → from-good-form-zero m f 
                                (chks-good-form 0 f (from-ends-pass-if _ h1)) ) 
