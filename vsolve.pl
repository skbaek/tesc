...
lit_arity(~ ((_ $ TERMS)), ARI) :- length(TERMS, ARI).
lit_arity((_ $ TERMS), ARI) :- length(TERMS, ARI).

pred_def_norm_or((FORM \/  ~ ATOM), FORM, ATOM) :- atomic_form(ATOM).
pred_def_norm_or((FORM_A \/ FORM_B), (FORM_A \/ FORM_C), ATOM) :- 
  pred_def_norm_or(FORM_B, FORM_C, ATOM).

pred_def_norm(PRD, ! FORM, ARI, ! NORM) :- 
  pred_def_norm(PRD, FORM, ARI, NORM).
pred_def_norm(PRD, FORM_A \/ FORM_B, ARI, PRD $ TERMS <> FORM) :- 
  pred_def_norm_or(FORM_A \/ FORM_B, FORM, (PRD $ TERMS)), 
  length(TERMS, ARI).
pred_def_norm(P, P $ TS <> F, A, P $ TS <> F) :- length(TS, A).
...
tup_inst(
  (CNM, false, FORM, some(introduced(predicate_definition_introduction,[new_symbols(naming, [PRD_ATOM])]))),
