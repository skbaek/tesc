module definitions (D : Set) (wit : D) where
...
_/_↦r_ : RA → Ftr → Rl → RA 
(R / f0 ↦r r) f1 = if (eq-ftr f0 f1) then r else R f1

_/_↦f_ : FA → Ftr → Fn → FA 
(F / f0 ↦f f) f1 = if (eq-ftr f0 f1) then f else F f1
...
from-chks-good-ftr k (nf m) h = <n-to-< _ _ h
...
chks-good {true} k (cons t ts) h0 = 
  chks-good _ _ (from-tr-band-lft _ _ h0) , chks-good _ _ (from-tr-band-rgt _ _ h0)
chks-good {false} k (var m) h0 = tt
chks-good {false} k (fun f ts) h0 =
  from-chks-good-ftr _ _ (from-tr-band-lft _ _ h0) , 
  chks-good _ _ (from-tr-band-rgt _ _ h0)
...
from-chks-good-form k (bct _ f g) h0 =  
  (from-chks-good-form k f (from-tr-band-lft _ _ h0)) , 
  (from-chks-good-form k g (from-tr-band-rgt _ _ h0))
...
from-chks-good-form k (rel r ts) h0 = 
  from-chks-good-ftr _ r (from-tr-band-lft _ _ h0) , 
  chks-good k ts (from-tr-band-rgt _ _ h0) 
...
not-and-lft : ∀ {A : Set} {B : Set} → ¬ (A ∧ B) → A → ¬ B  
...
ftr-eq-to-eq : ∀ f g → tr (eq-ftr f g) → f ≡ g
...
good-ftr-to-eq k r R rl h0 = 
  intro-ite-lem {Rl} (eq-ftr (nf k) r) (λ x → x ≡ R r) 
        ( λ h1 → 
...
good-to-termoid-val-eq F V k fn (fun f ts) h0 = 
  cong2 {Fn} {List D} {D} (λ x y → x y) {(F / nf k ↦f fn) f} {F f} 
    ( intro-ite-lem {Fn} (eq-ftr (nf k) f) (λ x → x ≡ F f) 
        ( λ h1 → 
...
fa-update-eq F k fn = eq-symm (tr-to-ite-eq {_} {eq-ftr (nf k) (nf k)} (≡-to-=n k k refl)) 
 
ra-update-eq : ∀ R k r → (R / nf k ↦r r) (nf k) ≡ r
ra-update-eq R k r = (tr-to-ite-eq {_} {eq-ftr (nf k) (nf k)} (≡-to-=n k k refl)) 

ru-sf-eq : ∀ R k r s → (R / nf k ↦r r) (sf s) ≡ R (sf s)
ru-sf-eq R k r s = fs-to-ite-ne {Rl} {eq-ftr (nf k) (sf s)} {r} tt
