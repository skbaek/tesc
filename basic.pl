...
; 
  TERM_O = #(NUM).
safe_subst_term(NUM, TERM, ^(FUN, TERMS_I), ^(FUN, TERMS_O)) :- !, 
...
subst_term(NUM, TERM, ^(FUN, TERMS_I), ^(FUN, TERMS_O)) :- !, 
...
justified(C, $pos(FORM), SC) :- 
  ...
  mk_par(C, VARS, TERM),
  safe_subst_form(TERM, ANTE, TEMP),
  TEMP == CONS.
